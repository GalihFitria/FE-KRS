    ...
    <div id="formModal" class="flex items-center justify-center bg-gray-800 bg-opacity-50 fixed inset-0">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <form action="{{route('Mahasiswa.store')}}" method="post">
                ...
                <input type="text" id="nama_mahasiswa" class="border w-full p-2 mb-2 rounded" name="nama_mahasiswa">

                <label for="kelas" class="block">Kelas</label>
                <select id="kelas" name="id_kelas" class="border w-full p-2 mb-2 rounded">
                    @foreach($kelas as $row)
                    <option value="{{$row['id_kelas']}}">{{$row['nama_kelas']}}</option>
                    @endforeach
                </select>

                <label for="prodi" class="block">Prodi</label>
                <select id="prodi" name="kode_prodi" class="border w-full p-2 mb-2 rounded">
