...
<body class="flex items-center justify-center h-screen bg-blue-100">
    ...
    <div id="formModal" class="flex items-center justify-center bg-gray-800 bg-opacity-50 fixed inset-0">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            <h2 class="text-xl font-bold mb-4 text-center">Tambah Data Kelas</h2>

            <form action="{{route('Kelas.store')}}" method="post">
                @csrf

                <label class="block">Id Kelas</label>
                <input type="text" id="idinput" class="border w-full p-2 mb-2 rounded" name="id_kelas">

                <label class="block">Nama Kelas</label>
                <input type="text" id="namaInput" class="border w-full p-2 mb-2 rounded" name="nama_kelas">

                <div class="flex justify-center space-x-4 mt-4">
                    <a href="{{route('Kelas.index')}}" class="bg-gray-400 text-white px-4 py-2 rounded hover:bg-gray-500 transition duration-200">
                        Batal
                    </a>
