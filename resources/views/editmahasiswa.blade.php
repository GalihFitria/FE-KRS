        ...
        <div id="formModal" class="flex items-center justify-center bg-gray-800 bg-opacity-50 fixed inset-0">
            <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                ...
                <form action="{{ route('Mahasiswa.update', $mahasiswa['npm']) }}" method="post">
                    ...
                    <input type="text" name="nama_mahasiswa" value="{{$mahasiswa['nama_mahasiswa']}}" id="editNamaMahasiswa" class="border w-full p-2 mb-2 rounded">

                    <label for="jurusan" class="block">Kelas</label>
                    <select id="jurusan" name="id_kelas" class="border w-full p-2 mb-2 rounded">
                        @foreach($kelas as $row)
                        <option value="{{ $row['id_kelas'] }}"
                            {{ $mahasiswa['id_kelas'] == $row['id_kelas'] ? 'selected' : '' }}>
                            {{ $row['nama_kelas'] }}
                        </option>
                        @endforeach
                    </select>
