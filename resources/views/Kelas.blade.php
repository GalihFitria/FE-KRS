                ...
                <table class="w-full mt-4 border-collapse border border-gray-300">
                    ...
                    <tbody id="kelasTable">
                        ...
                        <tr>
                            ...
                            <td class="border p-2">{{ $k['id_kelas'] }}</td>
                            <td class="border p-2">{{ $k['nama_kelas'] }}</td>

                            <td class="border p-2 text-center flex gap-2 justify-center">
                                <a href="{{ route('Kelas.edit', $k['id_kelas']) }}" class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600">Edit</a>

                                <form action="{{ route('Kelas.destroy', $k['id_kelas']) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
